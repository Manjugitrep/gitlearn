...
const handleRequestOtp = async () => {
  try {
    ...
    const response = await axios.post("http://localhost:9829/employee/forgotpassword", null, {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:9829/employee/validateotp", null, {
        params: { email, otp },
      });
